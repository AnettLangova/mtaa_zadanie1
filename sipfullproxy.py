...
# precompile regex (kvoli jednoduchsiemu udrziavaniu a modularnosti) 
...
# funkcia, ktora zo stringu vytvori jeho hexa hodnotu a upravenym stringom z quotechars po castiach podla width [=16] (ako vo Wiresharku)
...
# funkcia, ktora upravi string chars tak, ze vsetky nie alfanum znaky zmeni na '.'
...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def isDeclined(self):
        return bool(rx_decline.search(self.data[0]))

    # zaloguje vsetkych zaregistrovanych klientov (vypise meno a jeho IP adresu s portom)
    ...
    # v kode bolo jej pouzitie zakomentovane (ale pravdepodobne ide o prepis v data[0])
    ...
    # vymaze riadok z data, ak sa tam nachadza Route: ...
    ...
    # prepis v data a zmena vo Via, kde sa zmeni IP a port na hosta
    def addTopVia(self):
        branch = ""
        data = []
        for line in self.data:
    ...
    # kontrola, ci este registrovana uri neexpriovala (ak ano, tak sa vymaze)
    ...
    # na vypis info o jednej uri  z registrovanych
    ...
    # vytvori odpoved, ktoru odosle server klientovi
    ...
    def processRegister(self):
        ...
        if expires == 0:
            if fromm in registrar:
                # if registrar.has_key(fromm):
                del registrar[fromm]
                self.sendResponse("200 OK - VYMAZANY [REGISTRACIA EXPIROVALA]")
                return
        else:
            now = int(time.time())
        ...
        self.debugRegister()
        self.sendResponse("200 OK - REGISTROVANY [NOVA REGISTRACIA]")

    # vytvorenie invite s osetrenim
    def processInvite(self):
        ...
        if len(origin) == 0 or not origin in registrar:
            self.sendResponse("400 Bad Request - ERROR NA STRANE ODOSIELATELA [INVITE]")
            return
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                logging.debug("---\n<< server send [%d]:\n%s\n---" % (len(text), text))
                calling = True
                fromAdd = self.getOrigin()
                toAdd = self.getDestination()
                logging.info(f'User {fromAdd} called to user {toAdd}.')
            else:
                self.sendResponse("480 Temporarily Unavailable - PRIJEMCA NEREGISTROVANY [INVITE]")
        else:
            self.sendResponse("500 Server Internal Error - ERROR NA STRANE SERVERA [INVITE]")

    # potrvrdenie na invite
    def processAck(self):
        global calling
        logging.debug("--------------")
        ...
        if len(destination) > 0:
            ...
            if destination in registrar:
                ...
                logging.debug("<<< %s" % data[0])
                logging.debug("---\n<< server send [%d]:\n%s\n---" % (len(text), text))
                fromAdd = self.getDestination()
                toAdd = self.getOrigin()
                if calling:
                    logging.info(f'User {fromAdd} answered the call to user {toAdd}.')
    ...
    def processNonInvite(self):
        ...
        origin = self.getOrigin()
        if len(origin) == 0 or not origin in registrar:
            self.sendResponse("400 Bad Request - ERROR NA STRANE ODOSIELATELA [NONINVITE/BYE]")
            return
        destination = self.getDestination()
        if len(destination) > 0:
            logging.debug("destination %s" % destination)
            if destination in registrar and self.checkValidity(destination):
                ...
                fromAdd = self.getOrigin()
                toAdd = self.getDestination()
                logging.info(f'User {fromAdd} ended the call to user {toAdd}.\n')
            else:
                self.sendResponse("406 Not Acceptable - DANY PRIJEMCA NEREGISTROVANY [NONINVITE/BYE]")
        else:
            self.sendResponse("500 Server Internal Error - ERROR NA STRANE PRIJEMNCU [NONINVITE/BYE]")

    # vytvori a posle info o volajucom ()
    def processCode(self):
        ...
        if len(origin) > 0:
            ...
            if origin in registrar:
                ...
                logging.debug("<<< %s" % data[0])
                logging.debug("---\n<< server send [%d]:\n%s\n---" % (len(text), text))
                if self.isDeclined() and calling:
                    calling = False
                    toAdd = self.getDestination()
                    logging.info(f'User {toAdd} rejected the call.\n')

    # podla priznaku, spravy v data[0] sa urci, co sa ma vykonat na serveri
    def processRequest(self):
        ...
        if len(self.data) > 0:
            ...
            if rx_register.search(request_uri):
                ...
                self.sendResponse("200 OK - SUBSCRIBE")
            elif rx_publish.search(request_uri):
                self.sendResponse("200 OK - PUBLISH")
            elif rx_notify.search(request_uri):
                self.sendResponse("200 OK - NOTIFY")
    ...
    # override z BaseRequestHandler
    # metoda, ktora ma na starosti vsetko, co sa tyka obsluhy telefonovania
    def handle(self):
